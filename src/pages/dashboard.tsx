...
const DashboardPage: React.FC = () => {
  ...
  return (
    <>
      <Head>
        <title>Dashboard | FOH Pro</title>
        <meta name="description" content="Your FOH Pro dashboard" />
      </Head>
        ...
        <div className={`rounded-lg p-6 mb-6 border ${effectiveRole === UserRole.ADMIN ? 'bg-rose-500/10 border-rose-500/20' : effectiveRole === UserRole.EMPLOYEE ? 'bg-blue-500/10 border-blue-500/20' : 'bg-emerald-500/10 border-emerald-500/20'}`}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground mt-1">Welcome, {user?.displayName || user?.email || 'Test User'}!</p>
            </div>
            <div className={`px-4 py-2 rounded-full text-sm font-medium ${effectiveRole === UserRole.ADMIN ? 'bg-rose-500/20 text-rose-500' : effectiveRole === UserRole.EMPLOYEE ? 'bg-blue-500/20 text-blue-500' : 'bg-emerald-500/20 text-emerald-500'}`}>
              {effectiveRole === UserRole.ADMIN ? 'Manager' : effectiveRole === UserRole.EMPLOYEE ? 'Employee' : 'Customer'}
            </div>
          </div>
        </div>
